$(document).ready(function () {
  ...
  if (ids.length === 0) {
    $("#wishlist-lead").text(
      'Your wishlist is empty. Browse products and click "Add to wishlist" to save items here.',
    );
    return;
  }
  ...
  ids.forEach(function (id) {
    $.getJSON("/catalogue/" + id, { currency: currency }, function (p) {
      var card =
        ...
        p.id +
        '\');return false;" class="btn"><i class="fa fa-shopping-cart"></i> Add to cart</a>' +
        "</div>" +
        "</div>" +
        '<div class="text">' +
        ...
        p.price.toFixed(2) +
        "</p>" +
        '<p><a href="#" class="btn btn-xs btn-default remove-wish" data-id="' +
        p.id +
        '"><i class="fa fa-trash"></i> Remove</a></p>' +
        "</div>" +
        "</div>" +
        "</div>";
  ...
  $(document).on("click", ".remove-wish", function (e) {
    ...
    var remaining = JSON.parse(
      localStorage.getItem("soqoniWishlist") || "[]",
    ).length;
    if (remaining === 0) {
      $("#wishlist-lead").text(
        'Your wishlist is empty. Browse products and click "Add to wishlist" to save items here.',
